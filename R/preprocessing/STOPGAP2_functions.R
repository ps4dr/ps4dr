...
{
    load(gwas.file)
    #gwas.data <- gwas.filter(data) #.commented out due to error
...
ld.path="./STOPGAP2_LDResults/"){
    ...
    for (i in m.no) {
        ...
        if (nrow(ld.data) > 0) {
            ...
            ld.data <- subset(ld.data, ld.data$ID1 %in% gwas.ld.snps$rsid) #. changed SNP.gwas to ld.data$ID1
        } else {
            ld.data <- NA
        }
    ...
    ld.snps <- ld.snps.r2[, c("CHR2", "POS2", "ID2")] #.changed
    #. write.csv(ld.snps,file = "./STOPGAP2_LDResults/gwas_LD/ld.snps.csv",quote = F,row.names = F)
    #.ld.snps <- ld.snps[!duplicated(ld.snps),]
    #. used following linux command since R get terminated while handling duplicated function with 27 mil lines
    #. sort ld.snps.csv | uniq > uniq.ld.snps.csv
    #. vim to paste CHR.ld,POS.ld,SNP.ld in the first line
    #.ld.snps <- read.csv("./STOPGAP2_LDResults/gwas_LD/uniq.ld.snps.csv",sep = ",",header = T)
...
ld.r2.file="./STOPGAP2_LDResults/gwas_LD/ld.snps.r2.RData"){
    ...
    ld.snps.r2 <- subset(ld.snps.r2, ! (ID2 %in% snps.rm)) #.changed SNP.ld to ID2 #.check again
    ...
    miss.no1 <- which(ld.snps.r2$ID2 == ".") #.changed ld.snps.r2$SNP.ld to ld.snps.r2$ID2
    #ld.snps.r2$SNP.ld[miss.no1] <- paste(ld.snps.r2$CHR.ld[miss.no1], ld.snps.r2$POS.ld[miss.no1], sep=":")
    ld.snps.r2$ID2[miss.no1] <- paste(ld.snps.r2$CHR2[miss.no1], ld.snps.r2$POS2[miss.no1], sep = ":") #. changed
...
rsid.file = "./STOPGAP2_LDResults/rsID_Coordinates.txt"){
    ...
    header = TRUE, comment.char = "", sep = "\t", as.is = TRUE)
    names(coord)[names(coord) == "X.rsID_searched"] <- "rsID"
    ...
    gwas.data1 <- subset(gwas.data, ! (rsid %in% unique(ld.snps.r2$ID2))) #.changed ld.snps.r2$SNP.ld to ld.snps.r2$ID2
    ...
    gwas.data2$pos <- as.integer(as.character(gwas.data2$pos))
    gwas.data2 <- cbind(gwas.data2, gwas.data2)
    gwas.data2$r2 <- as.integer(1)
    gwas.data2$N_INDV <- as.integer(1) #.changed
    gwas.data2 <- gwas.data2[, c("chr", "pos", "chr", "pos", "N_INDV", "r2", "rsid", "REF", "ALT", "rsid", "REF", "ALT")] #.changed
    names(gwas.data2) <- names(ld.snps.r2)
    library(dplyr)
    ld.snps.r2 <- bind_rows(ld.snps.r2, gwas.data2)
    #ld.snps.r2 <- rbind(ld.snps.r2, gwas.data2) #. fails due to memory error
    save(ld.snps.r2, file = "./STOPGAP2_LDResults/gwas_LD/ld.snps.r2.updated.plusGWASsnps.RData", compress = TRUE)

    #.ld.snps <- ld.snps.r2[,c("CHR.ld", "POS.ld", "SNP.ld")]
    ld.snps <- ld.snps.r2[, c("CHR2", "POS2", "ID2")]
    names(ld.snps) <- c("CHR.ld", "POS.ld", "SNP.ld") #. changed
    ld.snps <- ld.snps[! duplicated(ld.snps),]
...
dis = 10000){
    ...
    load(gwas.file)
    load(ld.file)
    ld.snps.r2$POS1 <- as.numeric(ld.snps.r2$POS1) #. changed from ld.snps.r2$POS.gwas
    ld.snps.r2$POS2 <- as.numeric(ld.snps.r2$POS2) #. changed from ld.snps.r2$POS.ld
    ld.snps.r2 <- subset(ld.snps.r2, abs(POS1 - POS2) <= dis) #. changed from POS.gwas-POS.ld
    ...
    ld.snps <- ld.snps.r2[, c("CHR2", "POS2", "ID2")] #. changed from ld.snps.r2[,c("CHR.ld", "POS.ld", "SNP.ld")]
    names(ld.snps) <- c("CHR.ld", "POS.ld", "SNP.ld") #. changed
    ld.snps <- ld.snps[! duplicated(ld.snps),]
    save(ld.snps, file = "./STOPGAP2_LDResults/gwas_LD/ld.snps.updated.plusGWASsnps.500kb.RData", compress = TRUE)

    gwas.r2 <- merge(gwas.data[, c("rsid", "PUBMEDID", "pvalue")], ld.snps.r2[, c("ID1", "ID2", "r2", "CHR2", "POS2")],
    by.x = "rsid", by.y = "ID1", all = TRUE) #. changed from ld.snps.r2[,c("SNP.gwas", "SNP.ld", "r2", "CHR.ld", "POS.ld")
    #. by.y="SNP.gwas",
    names(gwas.r2) <- c("snp.gwas", "pubmedid", "pvalue", "snp.ld", "r2", "chr.ld", "pos.ld")
    gwas.r2 <- gwas.r2[! duplicated(gwas.r2),]
    system("mkdir LocusClustering") # running command 'mkdir LocusClustering' had status 127
...
frq.file="./Data/1KG_AF/gws.frq.RData"){
    ...
    by.x = c("CHR2", "POS2", "REF2", "ALT2"), # changed from by.x = c("CHR.ld","POS.ld", "REF.ld","ALT.ld"),
...
rdb.file="./Data/genomic_resource/regulomedb14.RData"){
    ...
    system("mkdir ./VarGeneMapping") #.changed
...
eQTL.multi.file="./Data/genomic_resource/eQTL_multi_9Tissues.RData"){
    ...
    by.y = "snp", all.x = TRUE, all.y = FALSE)
    #gwas.ld.eQTL.m$Tissue <- ifelse(is.na(gwas.ld.eQTL.m$Gene),NA,"AS;AT;HLV;Lu;MS;NT;SSEL;Th;WB") #. why did you comment it out?
...
Posigene.file="./Data/gencode_v19_clean.RData"){
    ...
    #Gene = "Single.eQTL.GeneType")) #. newely added, since this coulumn is required later
    ...
    gwas.ld.eQTL.s$Single.eQTL.GeneType <- trimWhiteSpace(gwas.ld.eQTL.s$Gene)
    ...
    #gwas.ld.rdb$SNP.ld <- trimWhiteSpace(gwas.ld.rdb$SNP.ld) #.since data set not present
    ...
    #                   by = c("SNP.ld","CHR.ld", "POS.ld"), all = TRUE) #. rdb  data is not present
...
var2gene.file = "./VarGeneMapping/Var2Gene_vepSimp.RData"){
    ...
    names(gwas.data) <- c("snp.gwas.orig", "snp.gwas", "pubmedid", "pvalue", "disease", "msh", "msh.tree", "msh.cat", "init.samp", "rep.samp", "source") #. removed "msh.id", no such column
    ...
    names(ld.snps.r2) <- c("chr.ld", "pos.ld", "ref.ld", "alt.ld", "chr.gwas", "pos.gwas", "N_INDV", "r2", "snp.gwas", "ref.gwas",
    "alt.gwas", "snp.ld", "af.1kg", "asn.af.1kg", "amr.af.1kg", "afr.af.1kg", "eur.af.1kg")
    ...
    # alreay filtered for this in "var2gene.filter" function why again? most importanly "chr.gencode" column is not present
    #. var2gene.vepSimp$chr.gencode <- NULL
    # incorrect order and names
    #. names(var2gene.vepSimp) <- c("gene","chr.ld","pos.ld", "snp.ld", "cat.rdb", "dhs.type",
    #                          "dhs.fpr","chiapet.cell","chiapet.type","fantom5.tissue","fantom5.fpr","fantom5.type",
    #                          "eqtl.ref","eqtl.tissue","eqtl.scoretype","eqtl.score","eqtl.gene.post","eqtl.snp.post",
    #                          "vep.phylop","vep.canonical","vep.conseq","vep.aa","vep.condel","gene.start","gene.end",
    #                          "distance.from.gene")

    names(var2gene.vepSimp) <- c("gene", "snp.ld", "chr.ld", "pos.ld", "dhs.type", "dhs.fpr", "chiapet.type",
    "CHIAPET.celltype", "CHIAPET.freq", "fantom5.tissue", "fantom5.fpr", "fantom5.type",
    "CHIC.Tissue", "CHIC.Type", "Single.eQTL.Tissue.p_value", "Single.eQTL.GeneType",
    "Tissue", "BIOTYPE", "cDNA_position", "CDS_position", "Protein_position", "vep.canonical",
    "vep.conseq", "vep.aa", "vep.condel", "chr", "cato.score", "cato.motifname", "cato.CellType",
    "deltaSVM.Top5.CellType", "deltaSVM.quantile.95p", "phyloP.score",
    "gene.start", "gene.end", "distance.from.gene")
...
severity.file = "./VarGeneMapping/VEP_Consequence_w_severity.txt"){
    ...
    for (i in chr.no) {
        ...
        gwas.r2.var2gene.withGene$score1 <- ifelse(gwas.r2.var2gene.withGene$phyloP.score >= 1.5, 1, 0) #.changed from phylop.score
        ...
        # 3: deltaSVM #. chage all deltasvm to deltaSVM
        gwas.r2.var2gene.withGene$deltaSVM.bin <- as.character(cut(gwas.r2.var2gene.withGene$deltaSVM.quantile.95p, breaks = c(0, 5.5, 9.7), right = FALSE, include.highest = TRUE)) #. cf gwas.r2.var2gene.withGene$deltaSVM.quantile.95
        ...
        gwas.r2.var2gene.withGene$score5[! is.na(gwas.r2.var2gene.withGene$CHIAPET.celltype)] <- 1 #. cf chiapet.celltype
        ...
        gwas.r2.var2gene.withGene$score7[! is.na(gwas.r2.var2gene.withGene$Single.eQTL.Tissue.p_value)] <- 2 #. cf .eqtl.tissue.pval
        gwas.r2.var2gene.withGene$score7.1 <- NA
        gwas.r2.var2gene.withGene$score7.1[! is.na(gwas.r2.var2gene.withGene$Single.eQTL.Tissue.p_value) & grepl(";", gwas.r2.var2gene.withGene$Single.eQTL.Tissue.p_value)] <- 2

        # 8: eqtl multi-tissue
        gwas.r2.var2gene.withGene$score8 <- NA
        gwas.r2.var2gene.withGene$score8[! is.na(gwas.r2.var2gene.withGene$multi.eqtl.tissue) & is.na(gwas.r2.var2gene.withGene$Single.eQTL.Tissue.p_value)] <- 3
        gwas.r2.var2gene.withGene$score8[! is.na(gwas.r2.var2gene.withGene$Single.eQTL.Tissue.p_value) &
            ! grepl(";", gwas.r2.var2gene.withGene$Single.eQTL.Tissue.p_value) &
            ! is.na(gwas.r2.var2gene.withGene$multi.eqtl.tissue)] <- 2
        ...
        gwas.r2.var2gene.withGene$score9[gwas.r2.var2gene.withGene$severity <= 10 &
            gwas.r2.var2gene.withGene$severity >= 6 &
            gwas.r2.var2gene.withGene$vep.conseq == "missense_variant" &
            is.neu] <- 3
        ...
        #gwas.r2.var2gene.withGene$var2gene.score <- NA
...
#. changes from /LocusClustering/gwas_r2.RData, since this doesnot has disease column
ld.data <- function(data.file="./GWAS_LD_var2gene/Mergedata_gwas_r2.RData"){
    load(data.file)
    ...
    gwas.r2.var2gene.full.ld <- gwas.r2[, c("snp.ld", "snp.gwas", "disease", "pvalue", "pubmedid", "r2", "chr.ld")]
    ...
    for (k in chr.no) {
        ...
        while (length(gwas.wait.snps) > 0) {
            ...
            if (nrow(wait.data) > 0) {
                ...
                if (nrow(cluster.i) > 0) { #. had to inroduce this condition to get rid of empty row errors from empty "cluster.i"
...
#"start.gwassnp", value = "rs991049
#Fine mapping of casual variants using PICS
#this function does not work in the cluster, due to inappropriate behaviour of count function from plyr package
fine.map.pics <- function(cluster.file = "./LocusClustering/STOPGAP_SNP_Cluster_chr", #. changed from STOPGAP_SNP_Cluster_
gwas.file="stopgap_4sources_dbSNP141_msh.RData"){
    ...
    for (i in chr.no) {
        ...
        stopgap.snp.cluster <- ld.clusters #. added since object name was different
...
sg.evid <- function(data) {
    ...
    leQTL <- ! is.na(data$Single.eQTL.Tissue.p_value) | ! is.na(data$multi.eqtl.tissue)
...
add.cluster <- function(data.file="./GWAS_LD_var2gene/Mergedata_VEPsimplified_withGene_r2_0.7_rmMHC_genescore_cluster_generank_bestLD_chr", #. added _cluster
cluster.file = "./LocusClustering/STOPGAP_SNP_Cluster_chr"){ #. added chr
    ...
    for (i in chr.no) {
        ...
        by = c("snp.ld", "snp.gwas", "chr.ld", "pvalue", "r2", "disease", "pubmedid"), all.x = TRUE, all.y = FALSE) #. removed "pos.ld", from by
...
Combine.bestld.data <- function(data.file="./GWAS_LD_var2gene/Mergedata_VEPsimplified_withGene_r2_0.7_rmMHC_genescore_generank_bestLD_cluster_chr"){
    #. added rm before MHC
...
mesh.gene <- function(data.file="./GWAS_LD_var2gene/Mergedata_VEPsimplified_withGene_r2_0.7_rmMHC_genescore_generank_bestLD_cluster.RData"){
    ...
    save(stopgap.bestld.gene.mesh, file = "./Mesh_gene/STOPGAP_r2_0.7_rmMHC_bestLD_gene_mesh.RData", compress = TRUE)
    #}
...
gencode.file="./Data/gencode_v19_clean.RData"
    ...
    orphanet$pvalue <- 0
    #.orphanet$best.gene <- omim$gene # where does this come from
    ...
    max.gene.score = 'gene.score.best', distance.from.gene = "gene.distance", #. assuming max.gene.score as best.gene.score
    CHIAPET.celltype = 'chiapet.cell', deltaSVM.Top5.CellType = "deltaSVM.cell",
    cato.CellType = "cato.cell", var2gene.score = "v2g.score"))
    #.stopgap.gene.mesh$eqtl.tissue.pvalue=ifelse((!is.na(stopgap.gene.mesh$Single.eQTL.Tissue.p_value) & !is.na(stopgap.gene.mesh$multi.eqtl.tissue)), paste(stopgap.gene.mesh$single.eqtl.tissue.pvalue,"multi-tissues",sep=";"),ifelse((!is.na(stopgap.gene.mesh$single.eqtl.tissue.pvalue) & is.na(stopgap.gene.mesh$multi.eqtl.tissue)),stopgap.gene.mesh$single.eqtl.tissue.pvalue, ifelse((is.na(stopgap.gene.mesh$single.eqtl.tissue.pvalue) & !is.na(stopgap.gene.mesh$multi.eqtl.tissue)),"multi-tissues",NA)))
    #. had to change since multi.eqtl.tissue column is not present
    stopgap.gene.mesh$eqtl.tissue.pvalue = stopgap.gene.mesh$Single.eQTL.Tissue.p_value

    #. stopgap.gene.mesh$single.eqtl.gene.type=NULL #absent
    #. stopgap.gene.mesh$multi.eqtl.tissue=NULL #absent
    stopgap.gene.mesh$Single.eQTL.Tissue.p_value = NULL
    stopgap.gene.mesh$vep.canonical = NULL
    stopgap.gene.mesh$chiapet.type = NULL
    stopgap.gene.mesh$CHIC.Type = NULL
    #. stopgap.gene.mesh$post=NULL #absent
    #  stopgap.gene.mesh$dhs.type=NULL
    #. stopgap.gene.mesh$max.gene.score=NULL # already changed to best.gene.score
    ...
    #> length(unique(stopgap.gene.mesh$gene.v19))
    #[1] 19116
    ...
    "chiapet.cell", "dhs.type", "dhs.fpr", "fantom5.tissue", "fantom5.fpr", "deltaSVM.quantile.95p", "deltaSVM.cell", "cato.score", "cato.motifname",
    "cato.cell", "phyloP.score", "chr.ld", "pos.ld", "snp.gwas.orig", "init.samp", "rep.samp", "msh.tree",
    "msh.cat", "af.1kg", "asn.af.1kg", "amr.af.1kg", "afr.af.1kg", "eur.af.1kg", "gene.start", "gene.end",
    "clust.init.gwassnp", "gwassnp.p.rank",
    "asso.count", "num.cluster", "num.gwas.snp",
    "num.ld.snp", "num.gene.cluster")

    #. "deltaSVM.95p" "vep.phylop"
    # has been changed to
    # "deltaSVM.quantile.95p", "phyloP.score"
    # "pics"  not found
...
revice.bestld <- function(gm.file = "./GWAS_LD_var2gene/Mergedata_VEPsimplified_withGene_r2_0.7_rmMHC_genescore_generank_bestLD_cluster.RData", #.changed from MHC
gencode.file="./Data/gencode_v19_clean.RData"){
    load(gm.file)
    load(gencode.file)
    ...
    #. had to change since multi.eqtl.tissue column is not present
    stopgap.bestld$eqtl.tissue.pvalue = stopgap.bestld$Single.eQTL.Tissue.p_value
    stopgap.bestld$single.eqtl.gene.type = NULL
    stopgap.bestld$multi.eqtl.tissue = NULL
    stopgap.bestld$Single.eQTL.Tissue.p_value = NULL
    ...
    var = c("gene", "gene.v19", "disease", "msh", "pvalue", "pubmedid", "source", "snp.gwas", "snp.ld", "r2", "evidence", "v2g.score",
    "gene.score", "gene.rank.max", "gene.rank.min", "gene.distance", "gene.best", "evidence.best",
    "cluster", "vep.conseq", "vep.severity", "vep.aa", "vep.condel", "eqtl.tissue.pvalue", "CHIC.Tissue",
    "chiapet.cell", "dhs.type", "dhs.fpr", "fantom5.tissue", "fantom5.fpr", "deltaSVM.quantile.95p", "deltaSVM.cell", "cato.score", "cato.motifname",
    "cato.cell", "phyloP.score", "chr.ld", "pos.ld", "snp.gwas.orig", "init.samp", "rep.samp", "msh.tree",
    "msh.cat", "af.1kg", "asn.af.1kg", "amr.af.1kg", "afr.af.1kg", "eur.af.1kg", "gene.start", "gene.end",
    ...
    #. [1] "deltaSVM.95p"    "vep.phylop"
    # has been changed to
    # "deltaSVM.quantile.95p", "phyloP.score"
    #  "gene.score.best","pics"  not found, so delted from var
